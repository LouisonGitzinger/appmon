...
aapt_path = "/Users/lgitzing/Library/Android/sdk/build-tools/27.0.2/aapt"
apktool_path = "/usr/local/lib/apktool"
zipalign_path ="/Users/lgitzing/Library/Android/sdk/build-tools/27.0.2/zipalign"
apksigner_path = "/Users/lgitzing/Library/Android/sdk/build-tools/27.0.2/apksigner"
...
def findLaunchableActivity(WORK_DIR, package_name, launchable_activity):
    launchable_activity_path = None
    for root, dirs, files in os.walk(os.path.join(WORK_DIR, package_name)):
        for dir in dirs:
            if "smali" in dir:
                launchable_activity_path = os.path.join(WORK_DIR, package_name, dir,
                                                        launchable_activity.replace(".", "/") + ".smali")
                if os.path.exists(launchable_activity_path):
                    return launchable_activity_path
...
try:
    ...
    apk_dump = subprocess.check_output([aapt_path, "dump", "badging", apk_path]).decode('utf-8')
    apk_permissions = subprocess.check_output([aapt_path, "dump", "permissions", apk_path]).decode('utf-8')
    ...
    try:
        ...
        # sys.exit(1)
    # launchable_activity = 'com.firamo.livewallpaper.burningskull.StartActivity'



    new_apk_path = WORK_DIR + "/" + package_name + ".apk"
    subprocess.call(["cp", apk_path, new_apk_path])
    subprocess.call([apktool_path, "--quiet", "decode", new_apk_path])
    subprocess.call(["mv", package_name, WORK_DIR])

    launchable_activity_path = findLaunchableActivity(WORK_DIR, package_name, launchable_activity)

    if not "uses-permission: name='android.permission.INTERNET'" in apk_permissions:
        print("[I] APK needs INTERNET permission__")
    ...
    unzip_output = subprocess.check_output(["unzip", LIB_FILE_PATH, "-d", lib_dir]).decode('utf-8')
    ...
    try:
        ...
        print(e)
    build_apk_output = subprocess.check_output([apktool_path, "build", os.path.join(WORK_DIR, package_name)]).decode('utf-8')
    ...
    subprocess.check_output([zipalign_path, "-v", "-p", "4", new_apk_path, aligned_apk_path]).decode('utf-8')

    align_verify = subprocess.check_output([zipalign_path, "-v", "-c", "4", aligned_apk_path]).decode('utf-8')
    align_verify.strip(" \r\n\t")
    ...
    sign_status = subprocess.check_output(
        [apksigner_path, "sign", "--verbose", "--ks", "appmon.keystore", "--ks-pass", "pass:appmon", "--out",
         signed_apk_path, aligned_apk_path]).decode('utf-8')

    if not "Signed" in sign_status:
        print("[E] APK signing error %s" % (sign_status))

    sign_verify = subprocess.check_output([apksigner_path, "verify", "--verbose", signed_apk_path]).decode('utf-8')
